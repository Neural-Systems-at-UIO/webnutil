...
def reconstruct_dzi_pyvips(zip_file_path: str):
    ...
    try:
        with zipfile.ZipFile(zip_file_path, "r") as zip_file:
            ...
            logger.info(
                f"DZI properties: Width={width}, Height={height}, TileSize={tile_size}, Format={img_format}"
            )
            ...
            tile_files = [
                ...
                if f.startswith(highest_level_dir + "/")
                and f.endswith(f".{img_format}")
            ]
                    ...
                    for x in range(tiles_across):
                        if (x, y) in tiles_dict:
                            ...
                            temp_tile_path = os.path.join(
                                temp_dir, f"tile_{x}_{y}.{img_format}"
                            )
                            ...
                            # Load with PyVIPS
                            tile_img = pyvips.Image.new_from_file(
                                temp_tile_path, access="sequential"
                            )
                            row_images.append(tile_img)
                        else:
                            # Create blank tile if missing
                            blank_tile = pyvips.Image.black(
                                tile_size, tile_size, bands=3
                            )
                            row_images.append(blank_tile)

                    # Join row horizontally
                    if row_images:
                        row_joined = pyvips.Image.arrayjoin(
                            row_images, across=len(row_images)
                        )
                ...
                if tile_images:
                    ...
                    if final_image.width > width or final_image.height > height:
                        final_image = final_image.crop(
                            0,
                            0,
                            min(width, final_image.width),
                            min(height, final_image.height),
                        )

                    # Convert to numpy array
                    # PyVIPS uses RGB, OpenCV expects BGR
                    np_array = np.ndarray(
                        buffer=final_image.write_to_memory(),
                        dtype=np.uint8,
                        shape=[
                            final_image.height,
                            final_image.width,
                    ...
                    logger.info(
                        f"PyVIPS DZI reconstruction completed: {np_array.shape}"
                    )
                    return np_array
    ...
    try:
        with zipfile.ZipFile(zip_file_path, "r") as zip_file:
            ...
            try:
                ...
                logger.error(
                    f"Error parsing DZI XML content from {dzi_file_name} in {zip_file_path}: {e}",
                    exc_info=True,
                )
                raise ValueError(f"Invalid DZI XML format in {zip_file_path}: {e}")
            except Exception as e:
                logger.error(
                    f"Unexpected error reading or parsing {dzi_file_name} in {zip_file_path}: {e}",
                    exc_info=True,
                )
                raise
            ...
            for item_name in zip_file.namelist():
                ...
                if len(parts) > 1 and parts[-1].startswith(
                    "0_0."
                ):  # Heuristic: check for top-left tile
                    try:
                        level = int(parts[-2])
                        if level > max_level:
                            max_level = level
            ...
            if (
                max_level == 0 and len(dzi_data["Image"]["Size"]["@Width"]) > tile_size
            ):  # if no levels, check if it's a single image
                logger.warning(
                    f"Could not determine max level, or it's a single tile image. Assuming level 0 for {zip_file_path}"
                )
                ...
                files_dir_name = os.path.splitext(dzi_file_name)[0] + "_files"
                if not files_dir_name in [
                    os.path.dirname(f) for f in zip_file.namelist()
                ]:  # if no _files dir, then it's a single tile
                    files_dir_name = ""

            highest_level_dir_name = (
                os.path.splitext(dzi_file_name)[0]
                + "_files"
                + (f"/{max_level}" if max_level > 0 else "")
            )
            logger.debug(
                f"Highest level directory identified as: {highest_level_dir_name}"
            )

            highest_level_tile_files = [
                f
                for f in zip_file.namelist()
                if f.startswith(highest_level_dir_name + "/")
                and (f.endswith(f".{img_format}"))
            ]
            ...
            if not highest_level_tile_files:
                ...
                highest_level_tile_files = [
                    f
                    for f in zip_file.namelist()
                    if f.startswith(files_dir_name_flat + "/")
                    and (f.endswith(f".{img_format}"))
                ]
                if highest_level_tile_files:
                    ...
                    logger.error(
                        f"No tile files found for format '{img_format}' in expected highest level directory: {highest_level_dir_name} or {files_dir_name_flat} within {zip_file_path}"
                    )
                    raise ValueError(
                        f"No highest level tile files found in {zip_file_path}"
                    )

            logger.info(
                f"Found {len(highest_level_tile_files)} tiles for the highest level."
            )
            ...
            for tile_file_path in highest_level_tile_files:
                try:
                    ...
                    tile_image = cv2.imdecode(
                        np.frombuffer(tile_contents, np.uint8), cv2.IMREAD_COLOR
                    )
                    if tile_image is None:
                        logger.warning(
                            f"Failed to decode tile: {tile_file_path} in {zip_file_path}. Skipping."
                        )
                        continue
                    ...
                    reconstructed_image[y_start:y_end, x_start:x_end, :] = tile_image[
                        :tile_y_end, :tile_x_end, :
                    ]
                    logger.debug(
                        f"Placed tile {tile_file_path} at ({x_offset}, {y_offset})"
                    )

                except Exception as e:
                    logger.error(
                        f"Error processing tile {tile_file_path} in {zip_file_path}: {e}",
                        exc_info=True,
                    )
        ...
        logger.error(
            f"Invalid or corrupted zip file: {zip_file_path} - {e}", exc_info=True
        )
        raise ValueError(f"Invalid or corrupted zip file: {zip_file_path} - {e}")
    except (
        FileNotFoundError
    ):  # Already caught and raised above, but good to be explicit
        raise
